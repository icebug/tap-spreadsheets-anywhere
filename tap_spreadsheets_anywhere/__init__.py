...
def sync(config, state, catalog):
    ...
    LOGGER.info(
        f"Processing {len(list(catalog.get_selected_streams(state)))} selected streams from Catalog"
    )
    for stream in catalog.get_selected_streams(state):
        LOGGER.info("Syncing stream:" + stream.tap_stream_id)
        catalog_schema = stream.schema.to_dict()
        table_spec = next(
            (x for x in config["tables"] if x["name"] == stream.tap_stream_id), None
        )
        if table_spec is not None:
            ...
            try:
                ...
                # Write final state only if we successfully completed the loop without exceptions.
                # This is important for ensuring that the state isn't emptied if no records are processed.
                singer.write_state(state)
            except Exception as e:
                # If an exception occurs during file processing, do NOT write state.
                # State for successfully processed files has already been written (line above in loop).
                # We don't write state here to avoid marking failed files as complete.
                LOGGER.error(
                    f'Error processing stream "{stream.tap_stream_id}": {e}',
                    exc_info=True,
                )
                raise

            LOGGER.info(
                f'Wrote {records_streamed} records for stream "{stream.tap_stream_id}".'
            )
        else:
            LOGGER.warn(
                f"Skipping processing for stream [{stream.tap_stream_id}] without a config block."
            )
    return
...
def main():
    ...
    crawl_paths = [
        x for x in args.config["tables"] if "crawl_config" in x and x["crawl_config"]
    ]
    if len(crawl_paths) > 0:  # Our config includes at least one crawl block
        LOGGER.info(
            "Executing experimental 'crawl' mode to auto-generate a table config per bucket."
        )
        tables_config = file_utils.config_by_crawl(crawl_paths)
        # Add back in the non-crawl blocks
        tables_config["tables"] += [
            x
            for x in args.config["tables"]
            if "crawl_config" not in x or not x["crawl_config"]
        ]
